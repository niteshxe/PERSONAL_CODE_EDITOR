...
let editorModels = {}; // Store models for each file
...
// FIXED: Create or get model for a specific file
function getOrCreateModel(filePath, content, language) {
  if (editorModels[filePath]) {
    return editorModels[filePath];
  }
  
  const model = monaco.editor.createModel(content, language);
  editorModels[filePath] = model;
  
  // Add change listener to this specific model
  model.onDidChangeContent(() => {
    if (activeFilePath === filePath) {
      fileTree[filePath] = model.getValue();
      saveToDB();
    }
  });
...
// FIXED: Clean up unused models
function cleanupModel(filePath) {
  if (editorModels[filePath]) {
    editorModels[filePath].dispose();
    delete editorModels[filePath];
  }
}
...
// FIXED: Load file content with proper model management
...
// FIXED: Delete file with model cleanup
function deleteFile(path) {
  if (confirm(`Are you sure you want to delete '${path}'?`)) {
    ...
    if (activeFilePath === path) {
      ...
      const placeholderModel = monaco.editor.createModel("", "javascript");
      editor.setModel(placeholderModel);
    }
    updateFileExplorer();
    saveToDB();
...
// FIXED: Delete folder with model cleanup
...
// FIXED: Rename item with model management
  ...
  if (newName && newName !== oldName) {
    ...
    Object.keys(fileTree).forEach((filePath) => {
      if (filePath.startsWith(oldPath)) {
        ...
        // Handle model renaming for files
        if (!filePath.endsWith("/") && editorModels[filePath]) {
          modelUpdates[newFilePath] = editorModels[filePath];
          delete editorModels[filePath];
        }
      } else {
    ...
    Object.keys(modelUpdates).forEach(newFilePath => {
      editorModels[newFilePath] = modelUpdates[newFilePath];
    });
...
function importFolder() {
  ...
  input.onchange = (e) => {
    ...
    Object.keys(editorModels).forEach(path => {
      cleanupModel(path);
    });
...
// FIXED: Add this function to handle database clearing with model cleanup
async function clearDatabase() {
  ...
  try {
    ...
    // Reset the editor with empty model
    const placeholderModel = monaco.editor.createModel("", "javascript");
    editor.setModel(placeholderModel);
